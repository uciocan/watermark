...
namespace Nop.Plugin.Misc.Watermark.Infrastructure
{
    public class FontProvider : IDisposable
    {
        ...
        public FontProvider(INopFileProvider fileProvider)
        {
            _customFonts = new Lazy<IReadOnlyDictionary<string, string>>(() =>
            {
                ...
                return fileProvider
                    ...
                    .ToDictionary(
                        filename => filename.Replace(fontsDirectoryPath, string.Empty),
                        filename => filename);
            });
        }
        ...
        /// <summary>
        /// Returns an SKTypeface by name. Caller must NOT dispose the returned instance — fonts are cached and reused.
        /// </summary>
